---
name: update-cli
description: Maintainer workflow for kaneo-cli. Refresh the pinned Kaneo OpenAPI baseline to the latest upstream release tag, find what was added, changed or removed, adapt the CLI, the consumer skill and tests, verify against a disposable instance, then open, review and merge the PR and finish the release including the Homebrew Formula bump. EXPLICIT INVOCATION ONLY, as /update-cli; never run it on your own initiative. It pushes, merges and publishes a release.
license: MIT
...
It references this fleet's `reviewer` and `worker` subagents and the `ci-triage` tool. When they are unavailable, use the harness's closest equivalents: an independent review is still mandatory. On Windows without `core.symlinks`, the `.claude/skills/update-cli` symlink checks out as a plain file; invoke this file directly.
...
- Replace `api/openapi.json` (and `api/authentication.md` if it changed) with the reviewed bytes; update `api/provenance.json` (SHA-256, `retrieved_at`, source commit/URLs, `operation_count_observed`, verified `main` commit) and the tag, date and commit hash in `docs/api/README.md`. Recheck every `supplements` entry in provenance against the new spec and update gaps that closed.
...
4. Commit the work with its Conventional Commit type (step 6.1), so the review sees it.
5. **Review (mandatory).** Run the `reviewer` agent on `git diff main...HEAD`. Fix every confirmed finding, or stop if one cannot be fixed cleanly, then rerun step 1 and commit the fixes.

## 6. Version stamp, PR, merge

1. Commit type: `feat:` for new operations or flags, `fix:` for corrections only, `docs:` for a behavior-free refresh, and never a breaking release without asking (see Autonomy). A multi-commit PR squash-merges under the PR title, so the PR title must carry the same type.
2. Stamp against an up-to-date `main` in a full clone (`git fetch --tags origin`; the planner refuses shallow clones). If `main` has an untagged releasable merge (its release run is still pending or failed), wait for or resolve that release first: stamping now would claim the same version twice. Then `just stamp-skill` sets `metadata.version` in `skills/kaneo-cli/SKILL.md` to the planned release. Commit it. `go run ./internal/cmd/release plan` must report `release: true` and the intended next tag, and pass the stamp gate; `just check` again.
...
1. Watch the `main` CI run for the merge commit: `gh run list --workflow CI --branch main --commit <merge-sha> --json databaseId --jq '.[0].databaseId'`, then `gh run watch <id> --exit-status` (use `ci-triage <id>` on failure). It tags `vX.Y.Z`, publishes archives, packages and the container image, then opens `chore(homebrew): update kaneo-cli vX.Y.Z`. A failure after publication follows the recovery section of `docs/releases.md`. Never re-tag, clobber or rerun blindly.
2. Find the Formula PR by its branch: `gh pr list --head chore/homebrew-vX.Y.Z --state open` (a title search does not match `chore(homebrew)`). Verify every `sha256` in its `Formula/kaneo-cli.rb` against the release's `kaneo-cli_X.Y.Z_checksums.txt` by exact filename. Compare parsed values in a script, not by eyeballing shell output. All four must match; a mismatch stops the run.
