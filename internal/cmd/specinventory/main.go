...
type generatedInventory struct {
	...
	SecretBody     map[string]bool      `json:"-"`
...
func main() {
	...
	bodyHelpOutput, err := format.Source([]byte(renderBodyHelpSource(inventory.BodyHelp, inventory.SecretBody)))
	if err != nil {
...
func buildInventory() (generatedInventory, error) {
	...
	secretBody, err := buildSecretBodyOperations(spec, operations)
	if err != nil {
		return generatedInventory{}, err
	}
...
func renderBodyHelpSource(bodyHelp map[string]string, secretBody map[string]bool) string {
	...
	source.WriteString("}\n\n")
	source.WriteString("// generatedSecretBodyOperations lists operations whose JSON request body carries\n")
	source.WriteString("// a credential, so a plain-text error may echo a submitted value back.\n")
	source.WriteString("var generatedSecretBodyOperations = map[string]bool{\n")
	for _, operationID := range sortedKeys(secretBody) {
		fmt.Fprintf(&source, "\t%s: true,\n", strconv.Quote(operationID))
	}
	source.WriteString("}\n")
	return source.String()
}

// secretPropertyName matches a request-body property name that carries a
// credential. token_endpoint_auth_method is excluded by secretProperty because
// it names an auth method rather than a credential.
var secretPropertyName = regexp.MustCompile(`(?i)token|secret|password|apikey|accesskey|webhookurl`)

func secretProperty(name string) bool {
	if name == "token_endpoint_auth_method" {
		return false
	}
	return secretPropertyName.MatchString(name)
}

// buildSecretBodyOperations collects the operations whose JSON request body
// schema declares, at any depth, a property naming a credential.
func buildSecretBodyOperations(document map[string]any, operations []generatedOperation) (map[string]bool, error) {
	renderer := bodyHelpRenderer{document: document, resolving: make(map[string]bool)}
	...
	for _, operation := range operations {
		...
		content := objectValues(body["content"])
		schema := objectValues(objectValues(content["application/json"])["schema"])
		if schema == nil {
			continue
		}
		...
		if found {
			secret[operation.OperationID] = true
		}
	}
	return secret, nil
}
...
func (r *bodyHelpRenderer) hasSecretProperty(schema map[string]any, references map[string]bool) (bool, error) {
	...
	var members []any
	members = append(members, values(schema["allOf"])...)
	members = append(members, values(schema["anyOf"])...)
	members = append(members, values(schema["oneOf"])...)
	if item := schema["items"]; item != nil {
	...
	for _, member := range members {
		memberSchema := objectValues(member)
		if memberSchema == nil {
			continue
		}
