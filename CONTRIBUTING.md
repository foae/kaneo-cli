...
Use Conventional Commit squash titles: `feat:`, `fix:`, `perf:`, `docs:`, `test:`, `refactor:`, `build:`, `ci:`, `chore:`. Mark breaking changes with `!` or a `BREAKING CHANGE:` footer. These drive automatic releases after activation; see [release rules](docs/releases.md). Do not hand-edit a version string or move a published tag. The one committed version, the agent skill's `metadata.version`, is written by `just stamp-skill`: after the final Conventional Commit of a releasable change (`feat`, `fix` or breaking) is on the branch, run it and commit the result, and give the squash title that same type. If the release type is decided at merge time, stamp for that type first. Otherwise the post-merge `just check` and release planner fail closed and a follow-up stamp PR is needed.
